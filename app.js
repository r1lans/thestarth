...
function submitContactForm() {
    ...
    if (!SHEETS_LOG_URL) {
        statusEl.textContent = 'Форма пока настраивается — попробуйте позже, или напишите нам в WhatsApp.';
        statusEl.style.color = 'var(--accent-warm)';
        return;
    }
...
function renderReviewsEmpty(message) {
    ...
    el.innerHTML = '<strong>Пока нет отзывов</strong>' + escapeHtml(message || 'Станьте первым — расскажите о своём опыте обучения.');
    wrap.appendChild(el);
...
async function loadReviews() {
    ...
    if (typeof FIREBASE_READY === 'undefined' || !FIREBASE_READY) {
        renderReviewsEmpty('Отзывы появятся здесь, как только их опубликует администратор.');
        return;
    }
    try {
        ...
        items.forEach((r, i) => {
            ...
            card.innerHTML = `
                <div class="stars" aria-label="${rating} из 5">${'★'.repeat(rating)}<span style="opacity:.28">${'★'.repeat(5 - rating)}</span></div>
                <p style="margin-bottom: 1.25rem;">${escapeHtml(r.text)}</p>
                <div class="review-author">
                    <div class="review-avatar">${escapeHtml((name[0] || '?').toUpperCase())}</div>
                    <div>
                        <strong>${escapeHtml(name)}</strong>
                        <div class="review-date">${r.createdAt ? new Date(r.createdAt).toLocaleDateString('ru-RU') : ''}</div>
                    </div>
                </div>
            `;
        ...
        if (showMoreBtn && items.length > 6) showMoreBtn.style.display = 'inline-block';
    } catch (err) {
        console.warn('Reviews failed to load:', err);
        renderReviewsEmpty('Не удалось загрузить отзывы. Обновите страницу чуть позже.');
    }
}
            ...
            if (news.length) {
                ...
                news.forEach(n => {
                    ...
                    item.innerHTML = `
                        ...
                        <h3 style="margin-bottom: 0.5rem;">${escapeHtml(n.title)}</h3>
                        <p style="color: var(--ink-soft); line-height: 1.6;">${escapeHtml(n.body)}</p>
                    `;
...
async function submitPublicReview() {
    ...
    if (!name || !text) {
        statusEl.textContent = 'Заполните имя и текст отзыва.';
        statusEl.style.color = 'var(--accent-warm)';
        return;
    }
    if (typeof FIREBASE_READY === 'undefined' || !FIREBASE_READY) {
        statusEl.textContent = 'Форма отзывов пока настраивается — попробуйте позже.';
        statusEl.style.color = 'var(--accent-warm)';
        return;
    }
    try {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (forgotForm) {
        forgotForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                errEl.textContent = 'Письмо отправлено! Проверьте почту (и папку "Спам").';
            } catch (err) {
                errEl.style.color = 'var(--accent-warm)';
    ...
    function translateFirebaseError_(code, message) {
        const map = {
            ...
            'permission-denied': 'Отказано в доступе — проверьте, что правила Firestore (firestore.rules) опубликованы правильно.',
        ...
        return map[code] || (message ? `Ошибка: ${message} (${code || 'без кода'})` : 'Что-то пошло не так. Попробуйте ещё раз.');
    }
        ...
        registerForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                batch.set(db.collection('users').doc(uid), {
                    name, phone, nickname, course, email,
                    createdAt: new Date().toISOString()
                });
                batch.set(db.collection('nicknames').doc(nickname), { uid });
                await batch.commit();

                logToSheet('log_signup', { name, phone, nickname, course, email });
